...
export class BookBorrowingItemsRepository extends BaseRepository<BookBorrowingItems> {
   ...
   */
  async bulkUpdate(items: DeepPartial<BookBorrowingItems>[]): Promise<BookBorrowingItems[]> {
    const preloadedItems: BookBorrowingItems[] = [];

    for (const item of items) {
      if (!item.id) {
        throw new Error('Thiếu ID ở một trong các phần tử cần cập nhật.');
      }

      const existing: boolean = await this.existsBy({ id: item.id });
      if (!existing) {
        throw new Error(`Không tìm thấy bản ghi với ID: ${item.id}`);
      }

      const preloaded: BookBorrowingItems = await this.preload(item);

      if (preloaded instanceof BookBorrowingItems) {
        preloadedItems.push(preloaded);
      }
    }
